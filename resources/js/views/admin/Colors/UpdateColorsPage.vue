<template>
  <div>
    <div class="main-admin-top container">
      ...
      <div class="form-group">
        <button @click="updateColors()" type="button" class="btn btn-success">Update</button>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
  	...
  	mounted() {
  		this.colorsId = this.$route.params.id;
  		this.showColors();
  	},
  	...
  	methods: {
  		async showColors() {
  			try {
  				const colors = await Colors.showColors(this.colorsId);
  				if (colors.status === 200) {
  					this.name = colors.data.name;
  					this.code = colors.data.code;
  				}
  			} catch (e) {
  		...
  		async updateColors() {
  			...
  			if (isValid) {
  				...
  				const colors = await Colors.updateColors(
  					{
  						name,
  						code
  					},
